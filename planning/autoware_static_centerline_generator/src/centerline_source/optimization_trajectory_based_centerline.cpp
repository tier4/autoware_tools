  ...
  const LaneletRoute & route) const
{
  if (!path_generator_node_) {
    ...
    autoware::path_generator::PathGenerator::RouteManagerData route_manager_input;
    route_manager_input.lanelet_map_bin_ptr = map_bin_ptr;
    route_manager_input.route_ptr = std::make_shared<LaneletRoute>(route);
    path_generator_node_->initialize_route_manager(route_manager_input, current_pose);
  }
}
